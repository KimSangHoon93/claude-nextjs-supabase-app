// 임시 더미 데이터 파일 - Phase 3 Task 007(DB 스키마 생성) 완료 후 전량 교체 예정
import type {
  AdminStats,
  GatherEvent,
  GatherParticipant,
  GatherUser,
} from "@/types/gather";

export const MOCK_USERS: GatherUser[] = [
  {
    id: "user-1",
    email: "host@example.com",
    name: "김주최",
    avatarUrl:
      "https://api.dicebear.com/9.x/avataaars/png?seed=kimjuchoi&backgroundColor=b6e3f4",
    role: "user",
    createdAt: "2025-01-10T09:00:00Z",
  },
  {
    id: "user-2",
    email: "participant1@example.com",
    name: "이참여",
    avatarUrl:
      "https://api.dicebear.com/9.x/avataaars/png?seed=leechamyeo&backgroundColor=ffd5dc",
    role: "user",
    createdAt: "2025-02-05T10:00:00Z",
  },
  {
    id: "user-3",
    email: "participant2@example.com",
    name: "박길동",
    avatarUrl:
      "https://api.dicebear.com/9.x/avataaars/png?seed=parkgildong&backgroundColor=c0aede",
    role: "user",
    createdAt: "2025-03-20T11:00:00Z",
  },
];

export const MOCK_EVENTS: GatherEvent[] = [
  // user-1 주최 이벤트 (5개)
  {
    id: "evt-1",
    title: "팀 점심 모임",
    description: "분기별 팀 점심 식사 모임입니다. 편하게 참여해주세요!",
    location: "강남구 선릉역 근처 한식당",
    eventDate: "2026-06-10T03:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/lunch-team/800/400",
    inviteCode: "abc123",
    status: "upcoming",
    createdBy: "user-1",
    participantCount: 5,
    createdAt: "2026-05-01T09:00:00Z",
    updatedAt: "2026-05-01T09:00:00Z",
  },
  {
    id: "evt-2",
    title: "개발팀 워크샵",
    description: "2026 상반기 개발팀 워크샵. 발표 자료를 준비해주세요.",
    location: "판교 테크노밸리 컨퍼런스홀",
    eventDate: "2026-07-20T01:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/dev-workshop/800/400",
    inviteCode: "def456",
    status: "upcoming",
    createdBy: "user-1",
    participantCount: 12,
    createdAt: "2026-05-10T14:00:00Z",
    updatedAt: "2026-05-10T14:00:00Z",
  },
  {
    id: "evt-4",
    title: "신입사원 환영회",
    description:
      "3월 신규 입사자 환영 파티입니다. 즐거운 시간 되셨길 바랍니다!",
    location: "마포구 홍대 루프탑 카페",
    eventDate: "2026-03-15T09:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/welcome-party/800/400",
    inviteCode: "jkl012",
    status: "ended",
    createdBy: "user-1",
    participantCount: 8,
    createdAt: "2026-03-01T10:00:00Z",
    updatedAt: "2026-03-01T10:00:00Z",
  },
  {
    id: "evt-5",
    title: "프로덕트 리뷰 세션",
    description: "Q2 프로덕트 리뷰 및 피드백 세션입니다. 팀 전원 참석 필수!",
    location: "을지로 공유오피스 세미나룸",
    eventDate: "2026-06-25T05:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/product-review/800/400",
    inviteCode: "mno345",
    status: "upcoming",
    createdBy: "user-1",
    participantCount: 9,
    createdAt: "2026-05-15T09:00:00Z",
    updatedAt: "2026-05-15T09:00:00Z",
  },
  {
    id: "evt-6",
    title: "연말 파티",
    description: "올 한 해 수고한 팀원들과 함께하는 연말 파티입니다 🎊",
    location: "서울 성수동 이벤트홀",
    eventDate: "2026-12-20T10:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/year-end-party/800/400",
    inviteCode: "pqr678",
    status: "upcoming",
    createdBy: "user-1",
    participantCount: 30,
    createdAt: "2026-05-20T08:00:00Z",
    updatedAt: "2026-05-20T08:00:00Z",
  },
  // user-1 참여 이벤트 (2개, 다른 사람이 주최)
  {
    id: "evt-3",
    title: "사내 해커톤",
    description: "24시간 아이디어 해커톤 - 지금 진행 중입니다!",
    location: "본사 3층 대회의실",
    eventDate: "2026-05-21T00:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/hackathon/800/400",
    inviteCode: "ghi789",
    status: "ongoing",
    createdBy: "user-2",
    participantCount: 20,
    createdAt: "2026-05-01T08:00:00Z",
    updatedAt: "2026-05-01T08:00:00Z",
  },
  {
    id: "evt-7",
    title: "디자인 스프린트",
    description: "신규 기능 UX 개선을 위한 이틀짜리 디자인 스프린트입니다.",
    location: "강남 위워크 7층",
    eventDate: "2026-06-03T01:00:00Z",
    coverImageUrl: "https://picsum.photos/seed/design-sprint/800/400",
    inviteCode: "stu901",
    status: "upcoming",
    createdBy: "user-3",
    participantCount: 6,
    createdAt: "2026-05-18T10:00:00Z",
    updatedAt: "2026-05-18T10:00:00Z",
  },
];

export const MOCK_PARTICIPANTS: GatherParticipant[] = [
  // evt-1
  {
    id: "par-1",
    eventId: "evt-1",
    userId: "user-1",
    role: "host",
    joinedAt: "2026-05-01T09:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-2",
    eventId: "evt-1",
    userId: "user-2",
    role: "participant",
    joinedAt: "2026-05-02T10:00:00Z",
    user: MOCK_USERS[1],
  },
  {
    id: "par-3",
    eventId: "evt-1",
    userId: "user-3",
    role: "participant",
    joinedAt: "2026-05-03T11:00:00Z",
    user: MOCK_USERS[2],
  },
  // evt-2
  {
    id: "par-6",
    eventId: "evt-2",
    userId: "user-1",
    role: "host",
    joinedAt: "2026-05-10T14:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-7",
    eventId: "evt-2",
    userId: "user-2",
    role: "participant",
    joinedAt: "2026-05-11T09:00:00Z",
    user: MOCK_USERS[1],
  },
  // evt-4
  {
    id: "par-8",
    eventId: "evt-4",
    userId: "user-1",
    role: "host",
    joinedAt: "2026-03-01T10:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-9",
    eventId: "evt-4",
    userId: "user-3",
    role: "participant",
    joinedAt: "2026-03-02T10:00:00Z",
    user: MOCK_USERS[2],
  },
  // evt-5
  {
    id: "par-10",
    eventId: "evt-5",
    userId: "user-1",
    role: "host",
    joinedAt: "2026-05-15T09:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-11",
    eventId: "evt-5",
    userId: "user-2",
    role: "participant",
    joinedAt: "2026-05-16T10:00:00Z",
    user: MOCK_USERS[1],
  },
  {
    id: "par-12",
    eventId: "evt-5",
    userId: "user-3",
    role: "participant",
    joinedAt: "2026-05-16T11:00:00Z",
    user: MOCK_USERS[2],
  },
  // evt-6
  {
    id: "par-13",
    eventId: "evt-6",
    userId: "user-1",
    role: "host",
    joinedAt: "2026-05-20T08:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-14",
    eventId: "evt-6",
    userId: "user-2",
    role: "participant",
    joinedAt: "2026-05-21T09:00:00Z",
    user: MOCK_USERS[1],
  },
  // evt-3 (user-2 주최, user-1 참여)
  {
    id: "par-4",
    eventId: "evt-3",
    userId: "user-2",
    role: "host",
    joinedAt: "2026-05-01T08:00:00Z",
    user: MOCK_USERS[1],
  },
  {
    id: "par-5",
    eventId: "evt-3",
    userId: "user-1",
    role: "participant",
    joinedAt: "2026-05-05T09:00:00Z",
    user: MOCK_USERS[0],
  },
  // evt-7 (user-3 주최, user-1 참여)
  {
    id: "par-15",
    eventId: "evt-7",
    userId: "user-3",
    role: "host",
    joinedAt: "2026-05-18T10:00:00Z",
    user: MOCK_USERS[2],
  },
  {
    id: "par-16",
    eventId: "evt-7",
    userId: "user-1",
    role: "participant",
    joinedAt: "2026-05-19T09:00:00Z",
    user: MOCK_USERS[0],
  },
  {
    id: "par-17",
    eventId: "evt-7",
    userId: "user-2",
    role: "participant",
    joinedAt: "2026-05-19T10:00:00Z",
    user: MOCK_USERS[1],
  },
];

export const MOCK_ADMIN_STATS: AdminStats = {
  todayEvents: 2,
  weekEvents: 8,
  monthEvents: 25,
  totalEvents: 120,
  todayUsers: 5,
  weekUsers: 32,
  totalUsers: 450,
};

export function getMockEventById(id: string): GatherEvent | undefined {
  return MOCK_EVENTS.find((event) => event.id === id);
}

export function getMockEventByInviteCode(
  code: string,
): GatherEvent | undefined {
  return MOCK_EVENTS.find((event) => event.inviteCode === code);
}

export function getMockParticipantsByEventId(
  eventId: string,
): GatherParticipant[] {
  return MOCK_PARTICIPANTS.filter((p) => p.eventId === eventId);
}

// 사용자가 참여한 모든 이벤트 반환 (주최자 + 참여자 포함)
export function getMockEventsByUserId(userId: string): GatherEvent[] {
  const eventIds = MOCK_PARTICIPANTS.filter((p) => p.userId === userId).map(
    (p) => p.eventId,
  );
  return MOCK_EVENTS.filter((e) => eventIds.includes(e.id));
}

// 사용자가 주최한 이벤트만 반환
export function getMockHostedEventsByUserId(userId: string): GatherEvent[] {
  const eventIds = MOCK_PARTICIPANTS.filter(
    (p) => p.userId === userId && p.role === "host",
  ).map((p) => p.eventId);
  return MOCK_EVENTS.filter((e) => eventIds.includes(e.id));
}

// 사용자가 참여자로 참가한 이벤트만 반환
export function getMockJoinedEventsByUserId(userId: string): GatherEvent[] {
  const eventIds = MOCK_PARTICIPANTS.filter(
    (p) => p.userId === userId && p.role === "participant",
  ).map((p) => p.eventId);
  return MOCK_EVENTS.filter((e) => eventIds.includes(e.id));
}
